    ...
    : QMainWindow(parent)
{
    ...
    connect(ui.buttonUndo, &QPushButton::clicked, &urManager, &UndoRedoManager::Undo);
    connect(ui.buttonRedo, &QPushButton::clicked, &urManager, &UndoRedoManager::Redo);

    connect(ui.canvas, &Canvas::colorSelected, this, &GraphicsEditor::onColorSelected);
    connect(ui.canvas, &Canvas::canvasChanged, this, &GraphicsEditor::onCanvasChanged);
    connect(&urManager, &UndoRedoManager::stateChanged, this, &GraphicsEditor::onUndoRedo);
...
void GraphicsEditor::onCanvasChanged(QPixmap pixmap)
{
    urManager.AddPixmap(pixmap);
}
void GraphicsEditor::onUndoRedo(QPixmap pixmap, bool undo, bool redo)
{
    if (!pixmap.isNull())
        ui.canvas->SetPixmap(pixmap);
    ui.buttonUndo->setEnabled(undo);
    ui.buttonRedo->setEnabled(redo);
    // update undo redo buttons states
